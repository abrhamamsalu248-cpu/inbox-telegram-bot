...
# Token እና Admin ID ከ Render Environment Variable ብቻ እንዲያነብ
BOT_TOKEN = os.environ.get("INBOX_BOT_TOKEN")
ADMIN_ID = os.environ.get("ADMIN_ID")

if not BOT_TOKEN:
  raise ValueError("⚠️ INBOX_BOT_TOKEN አልተገኘም! በ Render Environment ውስጥ ያስገቡት።")
...
def start_cmd(message):
  if message.chat.id == ADMIN_ID:
    bot.reply_to(
        message, "👑 ሰላም አድሚን! የ Inbox ቦትህ በ Render ላይ ዝግጁ ነው።"
    )
  else:
    bot.reply_to(
        message,
...
@bot.message_handler(
    ...
    content_types=["text", "photo", "voice", "document", "video", "audio"],
)
def handle_incoming_user_message(message):
  ...
  user_info = (
      f"👤 <b>ከ፦</b> {safe_name} ({safe_username})\n🆔 <b>ID፦</b>"
      f" <code>{user.id}</code>"
  )
